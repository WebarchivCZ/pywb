...
case $param in
		...
		20)
			ARCHIVE_YEAR=20
			SEARCH_ROOT_DIR="/mnt/archive/KWA2020"
			;;
		21)
			ARCHIVE_YEAR=21
			SEARCH_ROOT_DIR="/mnt/archive/KWA2021"
			;;
		22)
			ARCHIVE_YEAR=22
		...
		25)
			ARCHIVE_YEAR=25
			SEARCH_ROOT_DIR="/mnt/archive/KWA2025"
			;;
		26)
			ARCHIVE_YEAR=26
...
find ${SEARCH_ROOT_DIR} \
	\( -path '*/profile/*' -o -path '*/lost+found' \) -prune \
	-o -type f \( -name "*.warc.gz" -o -name "*.arc.gz" -o -name "*.warc" \) \
	-exec bash -c 'create_collection_structure "$0"' {} \;
